...
export function processSpell(params: ProcessSpellParams): ProcessSpellResult {
  ...
  if (!spellSlot || spellSlot.current <= 0) {
    const battleAction: BattleAction = {
      ...
      resultText: `${caster.name} намагався використати ${spell.name}, але немає доступних spell slots`,
      hpChanges: [],
      isCancelled: false,
    };
  ...
  if (spell.damageType === "damage" || spell.damageType === "all") {
    ...
    for (const { target, finalDamage } of targetDamages) {
      ...
      updatedTargets[targetIndex].currentHp = Math.max(
        BATTLE_CONSTANTS.MIN_DAMAGE,
        updatedTargets[targetIndex].currentHp - remainingDamage
      );

      // Оновлюємо статус
      if (updatedTargets[targetIndex].currentHp <= 0) {
        updatedTargets[targetIndex].status =
          updatedTargets[targetIndex].currentHp < 0 ? "dead" : "unconscious";
      }
    }
    ...
    for (const target of updatedTargets) {
      ...
      if (updatedTargets[targetIndex].status === "unconscious" && updatedTargets[targetIndex].currentHp > 0) {
        updatedTargets[targetIndex].status = "active";
      }
    }
  } else {
      ...
      if (dotDamage > 0 && additionalModifier.duration > 0) {
        updatedTargets[targetIndex] = {
          ...
          activeEffects: addActiveEffect(
            updatedTargets[targetIndex],
            {
              id: `spell-modifier-${spell.id}-${target.id}-${Date.now()}`,
  ...
  updatedCaster.spellSlots[spellLevel] = {
    ...updatedCaster.spellSlots[spellLevel],
    current: updatedCaster.spellSlots[spellLevel].current - 1,
  };

  // 6. Позначаємо що кастер використав дію
  updatedCaster.hasUsedAction = true;
  ...
  const hpChanges = updatedTargets.map((target, index) => {
    ...
    const change = spell.damageType === "heal"
      ...
      : (target.currentHp - originalTarget.currentHp);

    return {
      participantId: target.id,
      participantName: target.name,
      oldHp: originalTarget.currentHp,
  ...
  const battleAction: BattleAction = {
    ...
    actionDetails: {
      ...
      savingThrows: savingThrowsDetails.length > 0 
        ...
        : undefined,
      appliedEffects: additionalModifier.modifier && additionalModifier.duration
        ? updatedTargets.map((t) => ({
            id: `spell-modifier-${spell.id}-${t.id}`,
            name: `${spell.name} - ${additionalModifier.modifier}`,
            duration: additionalModifier.duration!,
          }))
        : undefined,
    },
    resultText: `${caster.name} використав ${spell.name}${spell.damageType === "damage" ? ` завдавши ${spellCalculation.totalDamage || 0} урону` : spell.damageType === "heal" ? ` вилікувавши ${spellCalculation.totalHealing || 0} HP` : ""}`,
    hpChanges,
    isCancelled: false,
  };
