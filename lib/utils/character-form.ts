 ...
 */
export function characterToFormData(character: Partial<Character>): CharacterFormData {
  return {
    basicInfo: {
      name: character.name || "",
    ...
    spellcasting: {
      ...
      spellSlots: character.spellSlots as Record<string, { max: number; current: number }> | undefined,
      knownSpells: (character.knownSpells as string[]) || [],
    },
    roleplay: {
      languages: (character.languages as string[]) || [],
      proficiencies: (character.proficiencies as Record<string, string[]>) || {},
...
): Omit<Character, "id" | "campaignId" | "createdAt" | "updatedAt" | "user" | "inventory"> {
  return {
