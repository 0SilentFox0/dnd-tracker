...
export function calculateAttackBonus(
  ...
  const statModifier =
    attack.type === "melee"
      ? attacker.modifiers.strength
      : attacker.modifiers.dexterity;
  ...
  for (const effect of attacker.activeEffects) {
    for (const effectDetail of effect.effects) {
      if (effectDetail.type === "attack_bonus" || effectDetail.type === "attack") {
...
export function hasAdvantage(
  ...
  if (attacker.race?.toLowerCase().includes("elf") && attack.type === "ranged") {
    return true;
  }
  ...
  for (const effect of attacker.activeEffects) {
    for (const effectDetail of effect.effects) {
      if (effectDetail.type === "advantage" || effectDetail.type === "advantage_attack") {
        return true;
      }
    }
  }
...
export function canPerformReaction(
  ...
  const reactionSkills = defender.activeSkills.filter((skill) =>
...
export function performReaction(
  ...
  const reactionAttack = defender.attacks[0];
  ...
  return {
    ...
    message: `${defender.name} виконує контр-удар на ${attacker.name}!`,
    updatedDefender,
  };
}
