...
export function processAttack(params: ProcessAttackParams): ProcessAttackResult {
  ...
  const isHit =
    ...
    (attackRoll.isCritical || attackRoll.totalAttackValue >= updatedTarget.armorClass);
  ...
  if (attackRoll.isCriticalFail && attackRoll.criticalEffect) {
    ...
    const battleAction: BattleAction = {
      id: `attack-${attacker.id}-${Date.now()}`,
      battleId,
      round: currentRound,
      actionIndex: 0,
      timestamp: new Date(),
      ...
      actionDetails: {
        ...
        targetAC: updatedTarget.armorClass,
        isHit: false,
        isCritical: false,
        isCriticalFail: true,
      },
      resultText: `${attacker.name} критично промахнувся! ${criticalEffectApplied.description}`,
      hpChanges: [],
      isCancelled: false,
    };
  ...
  if (!isHit) {
    ...
    const battleAction: BattleAction = {
      ...
      resultText: `${attacker.name} промахнувся по ${target.name}`,
      hpChanges: [],
      isCancelled: false,
    };
  ...
  const statModifier =
    attack.type === "melee"
      ...
      : updatedAttacker.modifiers.dexterity;
  ...
  updatedTarget.currentHp = Math.max(
    BATTLE_CONSTANTS.MIN_DAMAGE,
    updatedTarget.currentHp - remainingDamage
  );

  // Оновлюємо статус
  if (updatedTarget.currentHp <= 0) {
    updatedTarget.status = updatedTarget.currentHp < 0 ? "dead" : "unconscious";
  }
  ...
  if (
    ...
    // Потім віднімаємо з currentHp
    updatedAttacker.currentHp = Math.max(
      BATTLE_CONSTANTS.MIN_DAMAGE,
      updatedAttacker.currentHp - remainingReactionDamage
    );

    // Оновлюємо статус
    if (updatedAttacker.currentHp <= 0) {
      updatedAttacker.status =
        updatedAttacker.currentHp < 0 ? "dead" : "unconscious";
    }

    reactionAttackerHpChange = {
      oldHp: oldAttackerHp,
      newHp: updatedAttacker.currentHp,
    };
  }

  // 10. Позначаємо що атакуючий використав дію
  updatedAttacker.hasUsedAction = true;
  ...
  const battleAction: BattleAction = {
    ...
    actionDetails: {
      ...
      damageRolls: damageRolls.map((roll, idx) => ({
      ...
      resultText: `${attacker.name} завдав ${totalFinalDamage} урону ${target.name}${attackRoll.isCritical ? " (КРИТИЧНЕ ПОПАДАННЯ!)" : ""}${criticalEffectApplied ? ` - ${criticalEffectApplied.name}` : ""}${reactionTriggered ? ` | ${target.name} виконав контр-удар на ${reactionDamage} урону` : ""}`,
      hpChanges: [
        {
          participantId: target.id,
          participantName: target.name,
          oldHp: oldHp,
          newHp: updatedTarget.currentHp,
