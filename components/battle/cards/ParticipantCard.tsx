...
export function ParticipantCard({
  ...
  const hpPercent = (participant.combatStats.currentHp / participant.combatStats.maxHp) * 100;
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            <h3 className="font-semibold text-sm sm:text-base truncate">{participant.basicInfo.name}</h3>
            <Badge
          ...
          {participant.combatStats.tempHp > 0 && (
            <div className="text-xs text-yellow-600 dark:text-yellow-400 mt-1">
              Temp HP: {participant.combatStats.tempHp}
            </div>
          )}
          ...
          {participant.combatStats.status !== "active" && (
            <Badge variant="outline" className="mt-1 text-[10px]">
              {participant.combatStats.status === "unconscious" ? "💤 Непритомний" : "💀 Мертвий"}
            </Badge>
          )}

          {/* Активні ефекти */}
          {participant.battleData.activeEffects.length > 0 && (
            <div className="flex flex-wrap gap-1 mt-2">
              {participant.battleData.activeEffects.slice(0, 3).map((effect, idx) => (
                <Badge key={idx} variant="outline" className="text-[9px]">
                  {effect.name} ({effect.duration})
                </Badge>
              ))}
              {participant.battleData.activeEffects.length > 3 && (
          ...
          {(isDM || !isEnemy) && (
            <div className="text-xs text-muted-foreground mt-1">
              AC: {participant.combatStats.armorClass}
            </div>
          )}
        </div>
      </div>
