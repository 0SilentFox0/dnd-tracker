...
export function SkillCircleGroup({
  ...
  // щоб кола завжди відображалися
  const filteredSkills = isDMMode
    ? skills.map((skill, originalIndex) => ({ skill, originalIndex }))
    : (() => {
        const nonPlaceholderSkills = skills
          .map((skill, originalIndex) => ({ skill, originalIndex }))
          .filter(
            ({ skill }) => skill.id && !skill.id.startsWith("placeholder_")
          );
        
        // Якщо є не-placeholder скіли, показуємо тільки їх
        // Якщо всі скіли placeholder, показуємо всі (щоб кола відображалися)
        return nonPlaceholderSkills.length > 0
          ? nonPlaceholderSkills
          : skills.map((skill, originalIndex) => ({ skill, originalIndex }));
      })();
